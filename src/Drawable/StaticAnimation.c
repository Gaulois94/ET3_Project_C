#include "Dawable/StaticAnimation.h"
...
StaticAnimation* StaticAnimation_create(const SDL_Rect* dest, SDL_Texture* texture, uint32_t posX, uint32_t posY, uint32_t padX, uint32_t padY, uint32_t width, uint32_t height, uint32_t nX, uint32_t nMax, uint32_t currentN, uint32_t nbFrame)
{
	...
	StaticAnimation_init(self, dest, texture, posX, posY, padX, padY, width, height, nX, nMax, nbFrame);
	return self;
}
...
{
	Animation_init((Animation*)self, dest, texture, NULL);
	self->posX   = posX;
	self->posY   = posY;
	self->padX   = padX;
	self->padY   = padY;
